...
- added new commands:
  ...
  - $toggle <ignoreWarnings, checkInstrumentUsingBlocks> <true, false>
  - $cleanup OR: $cleanup cancel OR: $cleanup toggle *<true, false>
  - $stage <create, delete, load> <stage name> (replaces $setStage)
  - $convert <song> <.nbs, .mid, .txt>
  - $playmode <commandblocks>
- features & changes:
  - 1.21 support (dropped support for 1.20.6 and below)
  - added the ability to record noteblocks you hear and save it as a song using .txt format
  - SongPlayer can now "clean up" the stage after it is done playing songs if you are using gamemode method of playing commands ($cleanup)
  - Added the ability to ignore warnings when playing (such as not having enough noteblocks to play in survival mode)
  - You no longer unfly when switching gamemodes while you're playing
  - You can now save your own stage designs to SongPlayer, stored in .minecraft/SongPlayer/stages/
  - Better implementation of detecting when you get moved away from your stage while playing
  - If you attempt to load a song that's already paused, it just resumes the song instead of adding the same song to queue
  - You can convert song formats to other song formats using $convert. Useful if you want to edit a song or want it to work with other programs
  - SongPlayer can now play songs using command blocks instead of noteblocks ($playmode commandblocks)
- bug fixes:
  - Fixed a bug where riding an entity would confuse SongPlayer
```
### 3.2.2
```
